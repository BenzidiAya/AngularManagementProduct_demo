...
export class ProductService {
  ...
  constructor(private http : HttpClient) { }
    ...
    return this.http.patch<Product>(`http://localhost:8086/products/${product.id}`, {checked :! product.checked})
    }
  ...
  saveProduct(product: Product) : Observable<Product> {
    return this.http.post<Product>(`http://localhost:8086/products`, product)
  }

  getPoductById(productId: number):Observable<Product> {
